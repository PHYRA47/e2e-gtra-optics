...
│   └── viz.py                # reusable diagnostics: layout, spot, PSF, before/after, evolution
├── demos/
...
- **swap the algorithm** — drop in the Wiener + dual-NAFNet IRM, or a detection head for a
  perception task, without touching optics or optimizer.
...
| "the Jacobian … all that good stuff" | forward-mode `∂ℓ/∂θ` | `optimizer/lm.py` |
| "the algorithm part … different neural networks" | image restoration model | `algorithm/` |
| "connected by backpropagation" | AD split (fwd for J, bwd for ∇L_ε) + alternating LM/Adam | `optimizer/joint.py` |
| "visualize the start and optimized optics … progressive" | paper-style layout/spot + evolution recorder | `viz.py` |
| "GTRA reduces to TRA" | `w=1/FWP`, `ε′=centroid` special case | `bridge/gtra.py::tra_control_values` |
...
- Glass-catalog mesh constraints (`optimizer/constraints.py`).
...
1. **A new optics model.** Subclass `BaseOptics`, register trainable params in `__init__`,
   ...
   `optimizer/constraints.py` per the paper's §1 caveat.
...
4. **New constraints.** Add residual functions in `optimizer/constraints.py`; they concatenate onto
...
| (11)/S62 | LM update | `optimizer.lm.LevenbergMarquardt.step` |
| S64/S65 | damping-matrix running max | `optimizer.lm.LevenbergMarquardt._update_damping` |
| S3.3.1 | joint LM/Adam step | `optimizer.joint.JointOptimizer.step` |
