...
def tra_control_values(spot: SpotDiagram):
    """Return the (w, eps') that make GTRA *equal* to TRA -- the reduction of Q4.

    The paper (\"Relation to conventional TRA\") states GTRA collapses to TRA for
    one specific choice of control values:

        w    = 1 / (F * W * P)          (uniform scalar weight)
        eps' = eps_bar                  (per-field centroid, broadcast)

    Plugging these into  l_GTRA = sqrt(w) * (eps - eps')  gives exactly
    l_TRA = (1/sqrt(FWP)) * (eps - eps_bar). This function returns those control
    values so you can drive the *same* GTRA code path in TRA mode -- i.e. TRA is
    not a separate branch, it is GTRA with a fixed (w, eps').

    Returns
    -------
    w        : scalar tensor  = 1/(FWP)
    eps_prime: Tensor (2FWP,) = broadcast per-field centroid
    """
    xy = spot.xy                                   # (F,W,P,2)
    F, W, P, _ = xy.shape
    w = torch.tensor(1.0 / (F * W * P), dtype=xy.dtype)
    cen = spot.centroids()                         # (F,2)
    eps_prime = cen.reshape(F, 1, 1, 2).expand(F, W, P, 2).reshape(-1).detach()
    return w, eps_prime
...
def gtra_residuals(eps: torch.Tensor,
                   ...
                   eps_prime_override: Optional[torch.Tensor] = None,
    ...
    if eps0 is None:
        eps0 = eps.detach()
    if w_override is not None and eps_prime_override is not None:
        # explicit control values -> drive the SAME code path in any mode.
        # Passing tra_control_values(spot) here makes this return l_TRA exactly.
        w = torch.as_tensor(w_override, dtype=eps.dtype).reshape(())
        eps_prime = eps_prime_override.detach()
    else:
