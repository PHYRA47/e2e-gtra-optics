...
These encode the sanity checks used while building the package:
  ...
  * the toy optics spot shrinks under TRA-LM.
"""
import os
...
def _toy(rings=6, fields=(0.0, 15.0, 30.0)):
    ...
    return RotationallySymmetricOptics(s, epd=5.0, fields_deg=list(fields),
...
def test_raytrace_gradient_matches_fd():
    ...
    optics = _toy(rings=4, fields=(0.0,))
    th = optics.get_theta()
    f = lambda t: optics.spot_from_theta(t).pow(2).sum()
    g = tf.grad(f)(th)
    eps = 1e-6
...
def test_forward_and_backward_ad_agree():
    ...
    optics = _toy(rings=4, fields=(0.0, 15.0))
    th = optics.get_theta()
    Jf = tf.jacfwd(optics.spot_from_theta)(th)
    Jr = tf.jacrev(optics.spot_from_theta)(th)
...
def test_gtra_gradient_equals_task_gradient():
    optics = _toy(rings=5)
    th = optics.get_theta()
    F, W, P = optics.fields_deg.numel(), optics.wavelengths_um.numel(), optics.pupil.shape[0]
    ...
    g_true = tf.grad(lambda t: task(optics.spot_from_theta(t)))(th)
    eps0 = optics.spot_from_theta(th).detach()
    L = task(eps0); gL = tf.grad(task)(eps0)
    resid = lambda t: gtra_residuals(optics.spot_from_theta(t), L, gL, eps0=eps0)
...
def test_toy_spot_shrinks_under_lm():
    optics = _toy(rings=6)
    F, W, P = optics.fields_deg.numel(), optics.wavelengths_um.numel(), optics.pupil.shape[0]
    esr0 = optics.forward().effective_spot_radius().item()

    def resid(t):
        xy = optics.spot_from_theta(t).reshape(F, W, P, 2)
        c = xy.mean(dim=(1, 2), keepdim=True)
        return ((xy - c) / (F * W * P) ** 0.5).reshape(-1)

    lm = LevenbergMarquardt(resid, optics.get_theta(), lam0=1.0); lm.run(30)
    optics.set_theta(lm.theta)
    esr1 = optics.forward().effective_spot_radius().item()
    assert esr1 < 0.25 * esr0, (esr0, esr1)


def _toy_dispersive(dispersion=True, wavelengths=(0.4861, 0.5876, 0.6563),
                    rings=6):
    """Toy optics with N-BK7 Abbe data on the two glass elements."""
    ...
    return RotationallySymmetricOptics(s, epd=5.0, fields_deg=(0.0, 15.0, 30.0),
...
def test_dispersion_off_is_monochromatic():
    ...
    optics = _toy_dispersive(dispersion=False)
    # all wavelengths share n_after -> identical spots per wavelength
    xy = optics.forward().xy                    # (F, W, P, 2)
...
def test_chromatic_spot_spread():
    ...
    optics = _toy_dispersive(dispersion=True)
    xy = optics.forward().xy                                        # (F, W, P, 2)
...
def test_derived_apertures_contain_all_rays():
    ...
    optics = _toy_derived()
    sa = optics.semi_aperture
    r = optics.ray_probes()["r"]                     # (S,F,W,P)
...
def test_declared_aperture_overrides_and_reports_overflow():
    ...
    optics = _toy()                                  # the old hardcoded values
    rows = optics.aperture_report()
    assert [r["declared"] for r in rows] == [2.6, 2.8, 3.4, 3.8]
    assert optics.semi_aperture == [2.6, 2.8, 3.4, 3.8]
...
def test_probes_do_not_perturb_the_trace():
    ...
    optics = _toy_derived(rings=4)
    packed = optics._pack()
    xy0, v0 = optics._trace_packed(packed)
    xy1, v1, pk = optics._trace_packed(packed, probes=True)
    assert torch.equal(xy0, xy1) and torch.equal(v0, v1)
    th_a = optics.get_theta().clone().requires_grad_(True)
    g0 = torch.autograd.grad(optics.spot_from_theta(th_a).pow(2).sum(), th_a)[0]

    def loss(t):
        p = optics._pack().clone().to(t.dtype)
        p[optics.var_mask] = t
        return optics._trace_packed(p, probes=True)[0].reshape(-1).pow(2).sum()

    th_b = optics.get_theta().clone().requires_grad_(True)
...
def test_spacing_kinds_read_off_the_optics():
    ...
    optics = _toy_derived(rings=4)
    kinds = _C.spacing_kinds_from_optics(optics)
    assert len(kinds) == optics.ray_probes()["tz"].shape[0]
...
def test_ray_path_residual_activates_on_violation():
    ...
    optics = _toy_derived(rings=4)
    gc = _C.GeometricConstraints.from_optics(optics, min_glass=0.25, min_air=0.1)
    assert float(_C.ray_path_residuals(optics.ray_probes(), gc.tz_min, gc.tz_max).max()) \
        < 0.05, "start design should be near-compliant"
    th = optics.get_theta().clone()
    idx = torch.nonzero(optics.var_mask).squeeze(-1).tolist()
    t0 = optics.n_surfaces * (2 + optics.max_asph)          # thickness block offset
    slots = [i for i, g in enumerate(idx) if g >= t0]
    th[slots[0]] = 0.05                                  # 2.2 mm -> 0.05 mm
    optics.set_theta(th)
    bad = _C.ray_path_residuals(optics.ray_probes(), gc.tz_min, gc.tz_max)
...
def test_geometric_residuals_are_differentiable():
    ...
    optics = _toy_derived(rings=4)
    gc = _C.GeometricConstraints.from_optics(optics, min_glass=2.5)   # deliberately tight
    th = optics.get_theta().clone().requires_grad_(True)

    def f(t):
        p = optics._pack().clone().to(t.dtype)
        p[optics.var_mask] = t
        return gc(optics._trace_packed(p, probes=True)[2]).pow(2).sum()
...
def test_constraints_rescue_a_violating_design():
    ...
    to ~0 by producing a degenerate optics whose rays stop tracing, while the
    constrained run climbs back toward the bound.
    """
    optics = _toy_derived(rings=4, fields=(0.0, 20.0))
    gc = _C.GeometricConstraints.from_optics(optics, min_glass=0.25, min_air=0.1)
    F = optics.fields_deg.numel(); W = optics.wavelengths_um.numel()
    P = optics.pupil.shape[0]
    idx = torch.nonzero(optics.var_mask).squeeze(-1).tolist()
    t0 = optics.n_surfaces * (2 + optics.max_asph)
...
# ---------------------------------------------------------------------------
def test_optics_names_are_element_agnostic():
    """The public API must not say "lens" -- a surface stack may be diffractive."""
    import e2e_optics as E
    from e2e_optics import viz as _V
    assert hasattr(E, "RotationallySymmetricOptics")
    assert hasattr(_V, "plot_layout") and hasattr(_V, "compare_designs")
    # no NEW public name may contain "lens"
    offenders = [n for n in E.__all__ if "lens" in n.lower()
                 and n != "RotationallySymmetricLens"]
    assert not offenders, f"lens-specific public names remain: {offenders}"


def test_deprecated_aliases_still_resolve():
    """Renaming must not break existing user scripts."""
    import e2e_optics as E
    from e2e_optics import viz as _V
    assert E.RotationallySymmetricLens is E.RotationallySymmetricOptics
    assert _V.plot_lens_layout is _V.plot_layout
    assert _V.compare_lenses is _V.compare_designs
...
def test_spot_at_field_selection_is_explicit():
    ...
    from e2e_optics import viz as _V
    optics = _toy_derived(rings=4, fields=(0.0, 15.0, 30.0))
    th = optics.get_theta().clone()
    _, _, fi_auto, _ = _V._spot_at(optics, th)                      # worst field
    for want in range(optics.fields_deg.numel()):
        _, _, got, rms = _V._spot_at(optics, th, field_index=want)
        assert got == want, f"asked for field {want}, plotted {got}"
        assert rms > 0.0
    # the automatic choice must really be the largest RMS field
    r = optics.forward().rms_radius()
...
def test_worst_field_changes_during_a_run():
    ...
    recorder = _V.OptimizationRecorder(optics)
    LevenbergMarquardt(resid, optics.get_theta(), lam0=1.0).run(25, callback=recorder)
    worst = [_V._spot_at(optics, recorder.thetas[k])[2] for k in range(recorder.n_frames)]
...
def test_evolution_grid_has_one_row_per_field():
    ...
    optics = _toy_derived(rings=4, fields=(0.0, 20.0))
    recorder = _V.OptimizationRecorder(optics)
    recorder(optics.get_theta())          # recorder accepts a bare tensor
    fig = _V.plot_spot_evolution(optics, recorder, n_show=2)
    # 2 fields x 2 iterations
    assert len(fig.axes) == 4, f"expected a 2x2 grid, got {len(fig.axes)} axes"
    fig2 = _V.plot_spot_evolution(optics, recorder, n_show=2, fields=[1])
...
def test_field_convergence_curve_covers_every_field():
    ...
    recorder = _V.OptimizationRecorder(optics)
    for _ in range(3):
        recorder(optics.get_theta())
    fig = _V.plot_field_convergence(optics, recorder)
    ax = fig.axes[0]
